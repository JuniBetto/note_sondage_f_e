...
class _TaskEditorSheetState extends State<_TaskEditorSheet> {
  ...
  Future<void> _loadAssigneesForSelectedTeam() async {
    ...
    try {
      final loadedAssignees = await widget.loadAssignees(teamId);
      if (!mounted) {
        return;
      }
      final hasSelected = loadedAssignees.any(
        (option) => option.userId == _selectedAssigneeUserId,
      );
      final assignees =
          !hasSelected &&
              _selectedAssigneeUserId?.trim().isNotEmpty == true &&
              _selectedAssigneeLabel?.trim().isNotEmpty == true
          ? <TaskAssigneeOption>[
              TaskAssigneeOption(
                userId: _selectedAssigneeUserId!.trim(),
                label: _selectedAssigneeLabel!.trim(),
              ),
              ...loadedAssignees,
            ]
          : loadedAssignees;
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      AppSnackBar.showError(
        context,
        AppErrorMessageResolver.resolve(error, fallback: l10n.taskSaveError),
      );
    } finally {
      if (mounted) {
        setState(() {
                                ...
                                onChanged: (value) {
                                  setState(() {
                                    ...
                                    if (value) {
                                      ...
                                      _selectedTeamId ??= widget
                                          .availableTeams
                                          .firstOrNull
                                          ?.id
                                          ?.trim();
                                      unawaited(
                                        _loadAssigneesForSelectedTeam(),
                                      );
                                    }
                                  });
                                },
                    ...
                    TaskEditorSectionCard(
                      ...
                      child: Column(
                        children: [
                          ...
                          const SizedBox(height: 14),
                          Text(
                            taskReminderSectionLabel(context),
                            ...
                            SegmentedButton<TaskReminderAnchor>(
                              ...
                              segments: [
                                ButtonSegment(
                                  ...
                                  label: Text(
                                    taskReminderAnchorDueAtLabel(context),
                                  ),
                                ),
                                ButtonSegment(
                                  value: TaskReminderAnchor.startAt,
                                  label: Text(
                                    taskReminderAnchorStartAtLabel(context),
                                  ),
                                ),
                              ],
