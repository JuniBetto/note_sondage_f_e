  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: DataTable2(
        ...
        columns: headerTable
            .map(
              (header) => DataColumn2(label: Text(header), size: ColumnSize.M),
            )
            .toList(),
        rows: List<DataRow>.generate(dataTable.length, (index) {
          ...
          return DataRow(
            cells: [
              DataCell(Text(clockInfo.user.toString())),
              DataCell(Text(clockInfo.clockInTime.toString())),
              DataCell(Text(clockInfo.clockOutTime.toString())),
              DataCell(Text(clockInfo.timeWorked.toString())),
              DataCell(Text(clockInfo.teamName.toString())),
            ],
