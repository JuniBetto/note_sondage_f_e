...
class CreateSondageMobile extends StatefulWidget {
  final String? sondageId;
  final Function()? onsondageCreated;

  const CreateSondageMobile({super.key, this.onsondageCreated, this.sondageId});

  @override
  State<CreateSondageMobile> createState() => _CreateSondageMobileState();
}

class _CreateSondageMobileState extends State<CreateSondageMobile> {
  ...
  // Sposta items qui per mantenere lo stato
  ...
  void dispose() {
    ...
    // Dispose di tutti i controller delle opzioni
    for (var controller in items) {
      controller.dispose();
    }
    items.clear(); // Pulisci la lista dopo aver disposato i controller
    super.dispose();
  }
  ...
  void _removeItem(int index) {
    setState(() {
      items[index].dispose(); // Dispose del controller prima di rimuoverlo
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              ), // Spazio vuoto per separare il titolo dalle opzioni
                ...
                itemBuilder: (context, index) {
                  return Padding(
                    ...
                    child: CustomTextFieldImmersive(
                      ...
                      onChanged: (value) {
                        setState(() {
                          // Conta quanti campi vuoti ci sono
                          int emptyCount = items
                              .where((controller) => controller.text.isEmpty)
                              .length;

                          // Se questo campo ora ha testo e non ci sono più campi vuoti, aggiungi uno nuovo
                          if (value.isNotEmpty && emptyCount == 0) {
                            _addItem();
                          }
                          // Se ci sono più di 1 campo vuoto, rimuovi quelli extra (tranne l'ultimo)
                          else if (emptyCount > 1) {
                            for (int i = items.length - 2; i >= 0; i--) {
                              if (items[i].text.isEmpty && emptyCount > 1) {
                                _removeItem(i);
                                emptyCount--;
                              }
                            }
              ...
              Padding(
                ...
                child: ToggleTile(
                  title: localization.allowMultipleResponses,
                  value: isEnabled,
                  onChanged: (val) {
                    setState(() {
                      isEnabled = val;
                  ...
                  CustomAppButton(
                    ...
                    onPressed: () async {
                      ...
                      final result = await Navigator.push<Map<String, dynamic>>(
                        context,
                        MaterialPageRoute(
                          builder: (context) => const SelectTeamPage(),
                        ),
                      );

                      // Se viene selezionato un team, aggiorna lo stato
                      if (result != null) {
                        setState(() {
                          selectedTeam = result;
                        });
              ...
              ElevatedButton(
                onPressed: () {
                  if (_formKey.currentState?.validate() ?? false) {
                    // TODO: Implementare la logica di creazione del sondaggio
                    // quando SondageBloc e SondageEntity saranno pronti
                ...
                child: Text('${localization.create} ${localization.sondage}'),
              ),
            ],
          ),
        ),
